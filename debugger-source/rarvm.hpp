...
enum VM_RESULT {NONE, SKIP, RETURN_TRUE, RETURN_FALSE};
...
class RarVM:private BitInput
{
  private:
	...
	VM_RESULT ExecuteInstruction(); // DEBUGGER CODE
	void DecodeInstruction(VM_PreparedCommand *Cmd, char* output, uint outputSize); // DEBUGGER CODE
	void DumpAll(); // DEBUGGER CODE
    void DebugCommand(); // DEBUGGER CODE
    uint GetCurrentPC(); // DEBUGGER CODE
    bool SetBreakpoint(uint pc, bool state); // DEBUGGER CODE
	...
	// DEBUGGER CODE
	VM_PreparedCommand* firstCommand;
	VM_PreparedCommand* currentCommand;
	uint amountOfInstructions;
	uint amountOfInstructionsExecutable;
    bool* breakpoints;
    uint breakNext;
    bool outputPC;
