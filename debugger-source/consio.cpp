...
static DEBUGGING_MODE DebuggingMode = DEBUGGING_INTERACTIVE;
void enableDebugger() {
	Debugger = true;
}
bool isDebugging() {
...
void setDebuggingMode(DEBUGGING_MODE DebuggingMode) {
	::DebuggingMode = DebuggingMode;
}
MESSAGE_TYPE getMsgStreamType() {
	return MsgStream;
}
// Debugget Code
...
#if !defined(GUI) && !defined(SILENT)
char* GetInput(char* str, uint length) {
	
	bool EndOfFile;
#if defined(__GNUC__) || defined(sun)
	EndOfFile = (fgets(str, length , stdin) == NULL);
    int len = strlen(str);
    if(str[len-1] == '\n') // remove new line
        str[len-1] = '\x00';
#else
	...
	readAmount = SrcFile.Read(str, length - 1);
	EndOfFile = (readAmount == 0);
#endif
	if (EndOfFile) {
