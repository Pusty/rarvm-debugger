...
void RarVM::Execute(VM_PreparedProgram *Prg)
{
	...
	R[7] = VM_MEMSIZE;
    if (isDebugging() && getDebuggingMode() == DEBUGGING_DIAGNOSTIC) printf("Initial R7 Value = %u\n", VM_MEMSIZE);
	Flags = 0;
...
#define SET_IP(IP)                      \
  if ((IP)>=amountOfInstructions)                   \
    return(RETURN_TRUE);                \
  if (--amountOfInstructionsExecutable<=0)                  \
    return(RETURN_FALSE);               \
  currentCommand=firstCommand+(IP);


uint RarVM::GetCurrentPC() {
    return (unsigned int)(currentCommand - firstCommand);
}

bool RarVM::SetBreakpoint(uint pc, bool state) {
    if(pc < 0 || pc >= amountOfInstructions) return false;
    breakpoints[pc] = state;
    return true;
}
...
bool RarVM::ExecuteCode(VM_PreparedCommand *PreparedCode, uint CodeSize)
{
	...
	firstCommand = PreparedCode;
	amountOfInstructions = CodeSize;
    
    if(getDebuggingMode() == DEBUGGING_INTERACTIVE) {
        breakpoints = new bool[amountOfInstructions];
        for(int i=0;i<amountOfInstructions;i++)
            breakpoints[i] = false;
        //breakpoints[0] = true; // first breakpoint to stop at the start
        breakNext = 0; // break in 0 instructions
        outputPC = true; // output instruction pointer
    }

	amountOfInstructionsExecutable = 25000000;
	currentCommand = firstCommand;


	if (isDebugging() && getDebuggingMode() == DEBUGGING_DUMP) {
		DumpAll();
		return true; // prevent actual code execution
	}
	...
	while (1) {
		if (isDebugging()) {
			if (getDebuggingMode() == DEBUGGING_TRACE) {
                char debugString[100];
                debugString[0] = '\x00';
				DecodeInstruction(currentCommand, debugString, sizeof(debugString));
				printf("  [%04X] %s", GetCurrentPC(), debugString);
			}
			else if (getDebuggingMode() == DEBUGGING_INTERACTIVE) {
                if(breakNext==0 || (breakNext == 25000001 && breakpoints[GetCurrentPC()] == true)) {
                    if(outputPC == true) {
                        outputPC = false;
                ...
                if(breakNext != 0 && breakNext != 25000001)
                    breakNext--;
			}
		}
		VM_RESULT result = ExecuteInstruction();
        outputPC = true;
		...
		currentCommand++;
		--amountOfInstructionsExecutable;
	}
    
    if (isDebugging() && getDebuggingMode() == DEBUGGING_INTERACTIVE) {
        printf("Finished executing at 0x%08X with return status %d\n", GetCurrentPC(), returnValue);
        breakNext=0;
        while(breakNext == 0)
            DebugCommand();
        printf("\n");
...
void RarVM::DebugCommand() {
    printf("(dbg) ");
    char command[256];
    GetInput(command, sizeof(command));
    for(int i=0;command[i]!='\x00';i++) // upper case all characters
        command[i] = etoupper(command[i]);
    ...
    switch(command[0]) {
        case 'C':
            if(stricomp(command, "CONTINUE") == 0) {
                //if(!SetBreakpoint(GetCurrentPC(), false))
                //    printf("Can't continue at 0x%08X\n", GetCurrentPC());
                breakNext = 25000002;
                return;
            }
            break;
        case 'B':
            value1 = GetCurrentPC();
            if(stricomp(command, "BREAK") == 0 
            || sscanf(command,"BREAK 0X%x",&value1) == 1
            || sscanf(command,"BREAK %u",&value1) == 1) {
                value1 &= 0xFFFFFFFF;
                if(SetBreakpoint(value1, true))
                    printf("Placed a breakpoint at 0x%08X\n", value1);
                else
                    printf("Unable to place a breakpoint at 0x%08X\n", value1);
                return;
            }
            break;
        case 'Q':
            if(stricomp(command, "QUIT") == 0 ) {
                exit(0);
                return;
            }
            break;
        case 'S':
            value1 = 1;
            if(stricomp(command, "STEPI") == 0 
            || sscanf(command,"STEPI 0X%x",&value1) == 1
            || sscanf(command,"STEPI %u",&value1) == 1) {
                value1 &= 0xFFFFFFFF;
                //if(!SetBreakpoint(GetCurrentPC(), false))
                //    printf("Can't step at 0x%08X\n", GetCurrentPC());
                breakNext = value1;
                printf("Stepping %u Instructions\n", value1);
                return;
            }
            if(sscanf(command,"SET FLAGS 0X%x", &value1) == 1
            || sscanf(command,"SET FLAGS %u", &value1) == 1) {
                value1 &= 0xFFFFFFFF;
                printf("Set flags to 0x%08X\n", value1);
                Flags = value1;
                return;
            }
            if(sscanf(command,"SET R%u 0X%x",&value2, &value1) == 2
            || sscanf(command,"SET R%u %u",&value2, &value1) == 2) {
                if(value2 < 0 || value2 > 7) {
                    printf("Register %u does not exist\n", value2);
                    return;
                }
                value1 &= 0xFFFFFFFF;
                printf("Set register %u to 0x%08X\n", value2, value1);
                R[value2] = value1;
                return;
            }
            if(sscanf(command,"SET 0X%x 0X%x",&value2, &value1) == 2
            || sscanf(command,"SET 0X%x %u",&value2, &value1) == 2
            || sscanf(command,"SET %u 0X%x",&value2, &value1) == 2
            || sscanf(command,"SET %u %u",&value2, &value1) == 2) {
                value1 &= 0xFF;
                value2 &= 0xFFFFFFFF;
                if(value2 < VM_MEMSIZE+4) {
                    Mem[value2]=(byte)value1;
                    printf("Set memory 0x%08X to 0x%02X\n", value2, value1);
                }else
                    printf("Address 0x%08X out of range\n", value2);
                return;
            }
            break;
        case 'G':
            if(stricomp(command,"GET FLAGS") == 0) {
                printf("FLAGS = 0x%08X\n", Flags);
                return;
            }
            if(sscanf(command,"GET R%u",&value2) == 1) {
                if(value2 < 0 || value2 > 7) {
                    printf("Register %u does not exist\n", value2);
                    return;
                }
            ...
            if(sscanf(command,"GET 0X%x",&value2) == 1
            || sscanf(command,"GET %u",&value2) == 1) {
                value2 &= 0xFFFFFFFF;
                if(value2 < VM_MEMSIZE+4) {
                    printf("Memory at 0x%08X to 0x%02X\n", value2,  Mem[value2]);
                }else
                    printf("Address 0x%08X out of range\n", value2);
                return;
            }
        ...
        case 'D':
            value1 = GetCurrentPC();
            value2 = amountOfInstructions-value1;
            if(stricomp(command, "DISASSEMBLE") == 0 
            || sscanf(command,"DISASSEMBLE 0X%x 0X%x",&value2,&value1) == 2
            || sscanf(command,"DISASSEMBLE %u 0X%x",&value2,&value1) == 2
            || sscanf(command,"DISASSEMBLE 0X%x %u",&value2,&value1) == 2
            || sscanf(command,"DISASSEMBLE %u %u",&value2,&value1) == 2
            || sscanf(command,"DISASSEMBLE 0X%x",&value2) == 1
            || sscanf(command,"DISASSEMBLE %u",&value2) == 1) {
                char debugString[100];
                for(int i=value1;i<value1+value2 && i < amountOfInstructions;i++) {
                    debugString[0] = '\x00';
                    DecodeInstruction(&firstCommand[i], debugString, sizeof(debugString));
                    printf("%c [%04X] %s",(i == GetCurrentPC())?'>':' ', i, debugString);
                }
                printf("Disassemble from 0x%x to 0x%x\n", value1, (value1+value2-1));
                return;
            }
            value1 = -1;
            if(stricomp(command, "DELETE") == 0 
            || sscanf(command,"DELETE 0X%x",&value1) == 1
            || sscanf(command,"DELETE %u",&value1) == 1) {
                if(value1 == -1) {
                    for(int i=0;i<amountOfInstructions;i++)
                        breakpoints[i] = false;
                    puts("Removed all breakpoints");
                    //SetBreakpoint(GetCurrentPC(), true);
                    return;
                }
                value1 &= 0xFFFFFFFF;
                if(SetBreakpoint(value1, false))
                    printf("Removed the breakpoint at 0x%08X\n", value1);
                else
                    printf("Unable to place a breakpoint at 0x%08X\n", value1);
                //SetBreakpoint(GetCurrentPC(), true);
                return;
            ...
            break;
        case 'I':
            if(strnicomp(command,"INFO",4) == 0 && strchr(command,' ')!=NULL) {
                argv1 = strchr(command,' ');
                if(stricomp(argv1, " REGISTERS") == 0) {
                    for(int i=0;i<8;i++)
                        printf("R%u     0x%08X\n",i,R[i]);
                    printf("Flags  0x%08X\n", Flags);
                }else
                    printf("Invalid INFO parameter '%s'\n", &argv1[1]);
                return;
            }
            break;
        case 'H':
            if(stricomp(command,"HELP") == 0) {
                puts("                          HELP DIALOG                          ");
                puts("===============================================================");
                puts(" help                            - help dialog");
                puts(" info registers                  - information about registers");
                puts(" break  [<addr>]                 - set a breakpoint");
                puts(" delete [<addr>]                 - remove breakpoints");
                puts(" disassemble [<amount>] [<from>] - disassemble instructions");
                puts(" get flags                       - read the flags");
                puts(" get R<register>                 - read a register");
                puts(" get <addr>                      - read byte from memory");
                puts(" set flags <value>               - set the flags");
                puts(" set R<register>  <value>        - set a register to a value");
                puts(" set <addr> <value>              - set a byte in memory");
                puts(" stepi <amount>                  - step over instructions");
                puts(" continue                        - continue until next break");
                puts(" quit                            - exit the debugger");
                return;
            }
            break;
    }
...
void RarVM::DumpAll() {
	char decodedInstruction[100];
	for (uint i = 0;i < amountOfInstructions;i++) {
		DecodeInstruction(&firstCommand[i], decodedInstruction, sizeof(decodedInstruction));
		printf("%s", decodedInstruction);
	}
}

void RarVM::DecodeInstruction(VM_PreparedCommand *Cmd, char* output, uint outputSize) {
	char decodedParameter1[40];
	decodedParameter1[0] = '\x00';
	char decodedParameter2[40];
	decodedParameter2[0] = '\x00';
	char decodedOpcode[8];
	decodedOpcode[0] = '\x00';
	// Get addresses to quickly access operands.
	VM_PreparedOperand* Op1 = (&Cmd->Op1);
	VM_PreparedOperand* Op2 = (&Cmd->Op2);

	snprintf(decodedOpcode, sizeof(decodedOpcode), "%s%c", VM_Command_Names[Cmd->OpCode], Cmd->ByteMode?'b':' ');

	switch(Op1->Type) {
	case VM_OPREG:
		snprintf(decodedParameter1, sizeof(decodedParameter1), "R%01X", Op1->Data);
		break;
	case VM_OPINT:
		if(Cmd->ByteMode)
			snprintf(decodedParameter1, sizeof(decodedParameter1), "#0x%02X", (Op1->Data&0xFF));
		else
			snprintf(decodedParameter1, sizeof(decodedParameter1), "#0x%08X", Op1->Data);
		break;
	case VM_OPREGMEM:
		if(Op1->Addr == &Op1->Data)
			snprintf(decodedParameter1, sizeof(decodedParameter1), "[#0x%08X]", ((*Op1->Addr + Op1->Base)&VM_MEMMASK));
		else
			snprintf(decodedParameter1, sizeof(decodedParameter1), "[R%01X+#0x%08X]", (unsigned int)(Op1->Addr-&R[0]), ((Op1->Base)&VM_MEMMASK));
		break;
	}

	switch (Op2->Type) {
	case VM_OPREG:
		snprintf(decodedParameter2, sizeof(decodedParameter2), ", R%01X", Op2->Data);
		break;
	case VM_OPINT:
		if (Cmd->ByteMode)
			snprintf(decodedParameter2, sizeof(decodedParameter2), ", #0x%02X", Op2->Data&0xFF);
		else
			snprintf(decodedParameter2, sizeof(decodedParameter2), ", #0x%08X", Op2->Data);
		break;
	case VM_OPREGMEM:
		if (Op2->Addr == &Op2->Data)
			snprintf(decodedParameter2, sizeof(decodedParameter2), ", [#0x%08X]", ((*Op2->Addr + Op2->Base)&VM_MEMMASK));
		else
			snprintf(decodedParameter2, sizeof(decodedParameter2), ", [R%01X+#0x%08X]", (unsigned int)((Op2->Addr-&R[0])), ((Op2->Base)&VM_MEMMASK));
		break;
	}

	//snprintf(decodedParameter1, sizeof(decodedParameter1), "%02X %08X %08X", Op1->Type, Op1->Base, Op1->Data);
	//if(Op2->Type != VM_OPNONE)
	//	snprintf(decodedParameter2, sizeof(decodedParameter2), ", %02X %08X %08X", Op2->Type, Op2->Base, Op2->Data);

	snprintf(output, outputSize, "%-8s %s%s\n", decodedOpcode, decodedParameter1, decodedParameter2);
	//printf("[%03X] %s %02X %02X\n", (currentCommand - firstCommand), VM_Command_Names[currentCommand->OpCode], *Op1, *Op2);
}

VM_RESULT RarVM::ExecuteInstruction() {
#ifndef NORARVM
    // Get addresses to quickly access operands.
    uint *Op1=GetOperand(&currentCommand->Op1);
    uint *Op2=GetOperand(&currentCommand->Op2);
#endif
    switch(currentCommand->OpCode)
    {
#ifndef NORARVM
      case VM_MOV:
        SET_VALUE(currentCommand->ByteMode,Op1,GET_VALUE(currentCommand->ByteMode,Op2));
        break;
#ifdef VM_OPTIMIZE
      case VM_MOVB:
      ...
      case VM_CMP:
        {
          uint Value1=GET_VALUE(currentCommand->ByteMode,Op1);
          uint Result=GET_UINT32(Value1-GET_VALUE(currentCommand->ByteMode,Op2));
      ...
      case VM_ADD:
        {
          ...
          SET_VALUE(currentCommand->ByteMode,Op1,Result);
        }
        break;
#ifdef VM_OPTIMIZE
      case VM_ADDB:
      ...
      case VM_INC:
        {
          uint Result=GET_UINT32(GET_VALUE(currentCommand->ByteMode,Op1)+1);
          if (currentCommand->ByteMode)
            Result&=0xff;
          SET_VALUE(currentCommand->ByteMode,Op1,Result);
      ...
      case VM_XOR:
        {
          uint Result=GET_UINT32(GET_VALUE(currentCommand->ByteMode,Op1)^GET_VALUE(currentCommand->ByteMode,Op2));
          Flags=Result==0 ? VM_FZ:Result&VM_FS;
          SET_VALUE(currentCommand->ByteMode,Op1,Result);
        }
        break;
      ...
      case VM_CALL:
        R[7]-=4;
        SET_VALUE(false,(uint *)&Mem[R[7]&VM_MEMMASK], currentCommand -firstCommand+1);
        SET_IP(GET_VALUE(false,Op1));
		return SKIP;
      case VM_NOT:
        SET_VALUE(currentCommand->ByteMode,Op1,~GET_VALUE(currentCommand->ByteMode,Op1));
        break;
      case VM_SHL:
        {
          uint Value1=GET_VALUE(currentCommand->ByteMode,Op1);
          uint Value2=GET_VALUE(currentCommand->ByteMode,Op2);
      ...
      case VM_NEG:
        {
          ...
          // compiler warning.
          uint Result=GET_UINT32(0-GET_VALUE(currentCommand->ByteMode,Op1));
      ...
      case VM_XCHG:
        {
          uint Value1=GET_VALUE(currentCommand->ByteMode,Op1);
          SET_VALUE(currentCommand->ByteMode,Op1,GET_VALUE(currentCommand->ByteMode,Op2));
          SET_VALUE(currentCommand->ByteMode,Op2,Value1);
        }
        break;
      case VM_MUL:
        {
          uint Result=GET_VALUE(currentCommand->ByteMode,Op1)*GET_VALUE(currentCommand->ByteMode,Op2);
          SET_VALUE(currentCommand->ByteMode,Op1,Result);
        }
        break;
      case VM_DIV:
        {
          uint Divider=GET_VALUE(currentCommand->ByteMode,Op2);
          if (Divider!=0)
          {
            uint Result=GET_VALUE(currentCommand->ByteMode,Op1)/Divider;
            SET_VALUE(currentCommand->ByteMode,Op1,Result);
          }
        }
        break;
      case VM_ADC:
        {
          uint Value1=GET_VALUE(currentCommand->ByteMode,Op1);
          uint FC=(Flags&VM_FC);
          uint Result=GET_UINT32(Value1+GET_VALUE(currentCommand->ByteMode,Op2)+FC);
          if (currentCommand->ByteMode)
            Result&=0xff;
      ...
      case VM_STANDARD:
        ExecuteStandardFilter((VM_StandardFilters)currentCommand->Op1.Data);
        break;
#endif
      case VM_PRINT:
        break;
...
void RarVM::Prepare(byte *Code,uint CodeSize,VM_PreparedProgram *Prg)
{
  ...
  Prg->CmdCount=0;
  if (isDebugging() && getDebuggingMode() == DEBUGGING_DIAGNOSTIC) {
      if(XorSum != Code[0]) // checksum
         printf("The checksum (%02X) does NOT matches the expected value (%02X).\n", XorSum, Code[0]);
      else
         printf("The checksum (%02X) matches the expected value (%02X).\n", XorSum, Code[0]);
  }
