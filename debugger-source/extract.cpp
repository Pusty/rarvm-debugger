  ...
  *ArcFileNameW=0;
  ...
  if (ExactMatch || (SkipSolid=Arc.Solid)!=0)
  {
    ...
    if (!IsLink(Arc.NewLhd.FileAttr))
      if (Arc.IsArcDir())
      {
        if (!ExtrFile || Command=='P' || Command=='D' || Command=='E' || Cmd->ExclPath==EXCL_SKIPWHOLEPATH)
          return(true);
        if (SkipSolid)
        {
#ifndef GUI
      ...
      else
      {
        ...
        if ((Command=='P' || Command=='D') && ExtrFile)
		  if(Command =='D' && getMsgStreamType() == MSG_NULL)
			  CurFile.SetHandleType(FILE_HANDLENUL);
		  else
    ...
    if (ExtrFile)
    {
      ...
      if (!SkipSolid)
      {
        if (!TestMode && (Command!='P' && Command!='D') && CurFile.IsDevice())
        {
