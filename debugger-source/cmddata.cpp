...
void CommandData::ProcessSwitch(const char *Switch,const wchar *SwitchW)
{
  ...
  switch(etoupper(Switch[0]))
  {
    ...
    case 'T':
      ...
      {
	    case 'R':
			if (stricomp(&Switch[2], "ACE") == 0) {
				setDebuggingMode(DEBUGGING_TRACE);
				break;
			}
			break;
        case 'K':
    ...
    case 'D':
		if (Switch[2] == 0) {
			...
			if (stricomp(&Switch[1], "EBUGGER") == 0) {
				setDebuggingMode(DEBUGGING_INTERACTIVE);
				break;
			}
			if (stricomp(&Switch[1], "UMP") == 0) {
				setDebuggingMode(DEBUGGING_DUMP);
				break;
			}
			if (stricomp(&Switch[1], "IAGNOSTIC") == 0) {
				setDebuggingMode(DEBUGGING_DIAGNOSTIC);
				break;
			}
		}
      break;
    ...
    case 'C':
      ...
      }	else if(Switch[1] == 'H' && stricomp(&Switch[2], "ECK") == 0) {
        setDebuggingMode(DEBUGGING_CHECK);
        break;
      }
      break;
    case 'K':
...
void CommandData::OutHelp(RAR_EXIT ExitCode)
{
  ...
  static MSGID Help[]={
    ...
    MCHelpCmdP,MCHelpCmdT,MCHelpCmdV,MCHelpCmdX,MCHelpCmdD,MCHelpSw,MCHelpSwm,
	...
	MCHelpSwTRACE,MCHelpSwDUMP,MCHelpSwDIAGNOSTIC
#else
#endif
  };
